'use strict';
var servicename = 'research';

module.exports = function(app) {

    var dependencies = ['$q', '$timeout'];
    ...
    function service($q, $timeout) {
        ...
        var searchSong = function(searchFilter) {
            // console.log('Searching airlines for ' + searchFilter);
            var deferred = $q.defer();

            var matches = chansons.filter(function(song) {
                return song.title.toLowerCase().indexOf(searchFilter.toLowerCase()) !== -1; //we compare the research string to the songs title
            });
