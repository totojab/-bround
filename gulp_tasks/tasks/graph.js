...
gulp.task('graph', 'Generate a dependency graph', function() {
    var folderScript = './' + constants.clientFolder + '/scripts';
    var files = fs.readdirSync(folderScript);
    var targets = selectTargets(files);
    gutil.log(gutil.colors.yellow('For this task to succeed you must make sure graphviz is properly installed:' + gutil.colors.green(' `brew install graphviz`')));
...
graphviz.on('exit',function() {
    console.log('exiting');
        ...
        return g.output('png', constants.graph.outputName, function() {
